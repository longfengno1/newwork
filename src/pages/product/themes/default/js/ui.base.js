...
$(function () {
	...
	$(".price_old").each(function(){
		$(this).html("<s style='display:inline-block;margin-left:-9px;width:8px;'>&nbsp;&nbsp;&nbsp;</s>"+$(this).text());	
	});
	...
	$(".menu").hover(function(){
		...
		if(isindex && $("#headerCtner").hasClass("headerCtnerFixed")){
	...
	$(".menunavi").find(">li dt").hover(function(){
		...
		UI.laterEvent(function(){
			...
			firstime=false;				
			if(!$this[0].ddtop){
				var cateWrap = $(".cateWrap",dd).css("width",201);
				var promotimgWidth = 175;
				if($(".promotimg img",dd).length==0){
					promotimgWidth = 0;
				}
				if(cateWrap.height()>331){
					cateWrap.css("width",402).masonry({
						itemSelector: ".catebox",
						columnWidth: 201
					});
					$(".ddwrap",dd).addClass("ddwrap_bg_col1");
				}
				
				$(".ddwrap",dd).css({width:cateWrap.width()+promotimgWidth});		
				/*if($(".promotimg img",dd).length==0){
					$(".ddwrap",dd).css({width:cateWrap.width()});	
				}
				cateWrap.masonry({
					itemSelector: ".catebox",
					columnWidth: 201
				...
				if(ie6){					
					try{
						DD_belatedPNG.fixPng(dd.find("img.ie6png2")[0]);
					}catch(e){}	
				}
	...
	$(".mycart").hover(function(){
		...
		UI.laterEvent(function(){
			if($this.find("li").length==0){
				...
				//$this.find("dd").hide();
				$this.find("dd h3").hide();
			}else{
				//$this.find("dt").addClass("dt_hover");
				//$this.find("dd").show();
				$this.find("dd h3").show();
			}
			$this.find("dt").addClass("dt_hover");
			$this.find("dd").show();
		},200,this);
   ...
   $(".mycart .listwrap").each(function(k,v){
	...
	//Header及Menu置顶固定的功能
	var headerCtner = $("#headerCtner");
	$(window).bind("scroll",function(){
		var st = document.documentElement.scrollTop ? document.documentElement.scrollTop : document.body.scrollTop;
		if(headerCtner.attr("data-pos-stick")!="true") return;
		
		if(st > 112){
			//当浏览器向下滚动到主导航内容时，header变为置顶固定
			headerCtner.next(".headerPlaceHolder").css("height",headerCtner.height());
			headerCtner.addClass("headerCtnerFixed");
			if(isindex==true){
				$("#menu").find(".menulist").hide();
			}
		}
		else {
			headerCtner.next(".headerPlaceHolder").css("height",0);
			headerCtner.removeClass("headerCtnerFixed");
			if(isindex==true){
				$("#menu").find(".menulist").show();
			}
		}
	});
	...
	//返回顶部
	$(".item_topTop .toTop").click(function () {
        $($.browser.safari || document.compatMode == 'BackCompat' ? document.body : document.documentElement).animate({
            scrollTop: 0
        }, 200);
        return false;
	...
	//Hover展示右侧浮动栏对应的栏目
	$(".floatFunc .item").hover(
		function(){
			var $this = $(this);
			$this.addClass("item-curr").find(".popItem").css({
				right:55,
				opacity:0	
			}).stop().show().animate({right:30,opacity:1},{duration:400,queue:false});
		},
		function(){
			var $this = $(this);
			if($this.hasClass("hasInputEle")){return false;}
			$this.removeClass("item-curr").find(".popItem").animate({right:55,opacity:0},{duration:400,queue:false,complete:function(){
				$(this).hide();
			}});
		}
	);
	$(".floatFunc .intxt").live("focusin",function(){
		$(this).parents("li.item").addClass("hasInputEle");	
	}).live("focusout",function(){
		$(this).parents("li.item").removeClass("hasInputEle");		
	}); 
